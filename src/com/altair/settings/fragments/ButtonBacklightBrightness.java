...
public class ButtonBacklightBrightness extends CustomDialogPref<AlertDialog> implements
    ...
    public void updateSummary() {
        if (mButtonBrightness != null) {
            ...
            if (buttonBrightness == 0.0f) {
                setSummary(R.string.navkeys_backlight_summary_disabled);
            } else if (timeout == 0) {
                setSummary(R.string.navkeys_backlight_timeout_unlimited);
            } else {
                setSummary(getContext().getString(R.string.navkeys_backlight_summary_enabled_with_timeout,
                        getTimeoutString(timeout)));
            }
        } else if (mKeyboardBrightness != null &&
                mKeyboardBrightness.getBrightness(true) != 0.0f) {
            setSummary(R.string.navkeys_backlight_summary_enabled);
        } else {
            setSummary(R.string.navkeys_backlight_summary_disabled);
        }
    }
    ...
    private void handleTimeoutUpdate(int timeout) {
        if (timeout == 0) {
            mTimeoutValue.setText(R.string.navkeys_backlight_timeout_unlimited);
        } else {
